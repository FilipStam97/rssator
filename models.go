...
type Post struct {
	...
	Description *string   `json:"description"` //we dont use sqql.ullString since its a nested struct and we will not get a good result we will get a nested structure)
	// so we use *string so it will parse to either string or null (since the marshaling json in go works like that)
